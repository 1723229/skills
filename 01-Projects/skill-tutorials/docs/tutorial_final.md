...
**feishu-doc-orchestrator** 是一个企业级的 Markdown 到飞书文档转换解决方案。采用**编排模式**（Orchestration Pattern），将文档创建流程分解为5个独立的子技能，通过文件传递数据，实现：
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('doc_creation.log'),
...
class FeishuDocCreator:
    ...
    def create_doc(self, md_file: str, title: str = None, retry: int = 0) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": "URL not found in output"}
...
```python
# 修改 feishu-block-adder/scripts/block_adder.py
BATCH_SIZE = 20  # 从10改为20，减少API调用次数
```
...
| 技能 | 组合方式 | 说明 |
|------|----------|------|
| feishu-wiki-orchestrator | 替代选择 | 创建到知识库（而非云盘） |
| content-extractor | 上游处理 | 抓取内容后保存为 Markdown，再用本技能转换 |
| document-hub | 统一入口 | 本技能是 document-hub 的一部分 |
...
| 问题 | 现象 | 解决方案 |
|------|------|----------|
| `pip install` 权限不足 | PermissionError | 使用 `pip install --user` 或虚拟环境 |
| `ModuleNotFoundError` | 模块未找到 | 使用 `python -m pip install` |

### 8.2 配置问题

| 问题 | 现象 | 解决方案 |
|------|------|----------|
| Config file not found | 找不到配置文件 | 检查 `~/.openclaw/.claude/feishu-config.env` |
| Failed to get token | Token 获取失败 | 确认 App ID/Secret 正确，应用已发布 |
| Permission denied | 权限不足 | 检查应用权限设置 |
...
| 技能 | 关系 | 说明 |
|------|------|------|
| feishu-wiki-orchestrator | 替代选择 | 直接在知识库创建文档 |
| feishu-doc-creator | 简化版 | 单技能实现，无编排 |
| content-extractor | 上游 | 抓取外部文章内容 |
